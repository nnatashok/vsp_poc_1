...
def analyze_youtube_workout(youtube_url, cache_dir='cache_19', force_refresh=False):
    """
    Analyzes a YouTube workout video and categorizes it according to various fitness spirits.
    ...
    Returns:
        dict: Workout spirit information
    """
    # API keys
    ...
    # Now send the metadata to OpenAI for workout spirit classification
    try:
        analysis = classify_workout_with_openai(oai_client, metadata)
        cache_data(analysis, analysis_cache_path)
        return analysis
    except Exception as e:
        return {"error": f"Failed to classify workout spirits: {str(e)}"}
...
from main19_spirits_and_schema import create_classification_prompt, RESPONSE_FORMAT
...
def classify_workout_with_openai(oai_client, metadata):
    """Use OpenAI to classify workout video spirits based on metadata with retry mechanism for rate limits."""
        ...
        try:
            response = oai_client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user",
                     "content": f"Analyze this workout video metadata and classify its spirit according to the schema:\n\n{formatted_metadata}"}
                ]
            )
            ...
            if "rate_limit_exceeded" in error_str or "Rate limit reached" in error_str:
                ...
                if retry_attempt == max_retries - 1:
                    return {"error": f"Error classifying workout spirits with OpenAI after {max_retries} retries: {str(e)}"}
            else:
                # If it's not a rate limit error, don't retry
                return {"error": f"Error classifying workout spirits with OpenAI: {str(e)}"}

    # This should only happen if we exhaust all retries
    return {"error": "Failed to classify workout spirits after maximum retry attempts"}


# Usage example
if __name__ == "__main__":
    # Add force_refresh=True to ignore cache and create a new analysis
    result = analyze_youtube_workout("https://www.youtube.com/watch?v=RKYhcPV2dbA", force_refresh=True)
