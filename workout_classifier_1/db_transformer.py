...
def extract_equipment_info(equipment_list):
    ...
    sorted_equipment = sorted(equipment_list, key=lambda x: x.get("confidence", 0), reverse=True)
...
def extract_spirit_info(spirits):
    ...
    secondary_spirit = sorted_spirits[1]["name"] if len(sorted_spirits) > 1 else None
...
def extract_vibe_info(vibes):
    ...
    secondary_vibe = sorted_vibes[1]["name"] if len(sorted_vibes) > 1 else None
