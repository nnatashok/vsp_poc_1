...
from main18_prompt_and_schema import CLASSIFICATION_PROMPT, RESPONSE_FORMAT, USER_PROMPT


def analyze_youtube_workout_equipment(youtube_url, cache_dir='cache_18', force_refresh=False):
    """
    Analyzes a YouTube workout video and identifies what equipment is needed.
    ...
    Returns:
        dict: Equipment information with confidence scores
    """
    # API keys
    ...
    analysis_cache_path = os.path.join(cache_dir, f"{video_id}_equipment_analysis.json")
    if os.path.exists(analysis_cache_path) and not force_refresh:
        try:
            with open(analysis_cache_path, 'r') as f:
                analysis = json.load(f)
            print(f"Loaded equipment analysis from cache: {analysis_cache_path}")
            return analysis
        except Exception as e:
            print(f"Error loading cached equipment analysis: {str(e)}. Proceeding with fresh analysis.")
    ...
    # Now send the metadata to OpenAI for equipment classification
    try:
        analysis = classify_equipment_with_openai(oai_client, metadata)
        cache_data(analysis, analysis_cache_path)
        return analysis
    except Exception as e:
        return {"error": f"Failed to identify equipment: {str(e)}"}
...
def classify_equipment_with_openai(oai_client, metadata):
    """Use OpenAI to identify equipment needed in workout video based on metadata with retry mechanism for rate limits."""
        ...
        try:
            ...
            if "rate_limit_exceeded" in error_str or "Rate limit reached" in error_str:
                ...
                if retry_attempt == max_retries - 1:
                    return {"error": f"Error identifying equipment with OpenAI after {max_retries} retries: {str(e)}"}
            else:
                # If it's not a rate limit error, don't retry
                return {"error": f"Error identifying equipment with OpenAI: {str(e)}"}

    # This should only happen if we exhaust all retries
    return {"error": "Failed to identify equipment after maximum retry attempts"}


# Usage example
if __name__ == "__main__":
    # Add force_refresh=True to ignore cache and create a new analysis
    result = analyze_youtube_workout_equipment("https://www.youtube.com/watch?v=zZD1H7XTTKc", force_refresh=True)
